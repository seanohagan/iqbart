...
void allsuff(tree& x, xinfo& xi, dinfo& di, tree::npv& bnv, std::vector<sinfo>& sv,
   pinfo& pi, const std::vector<double>& latent_nu)
{
   ...
   for(size_t i=0;i<di.n;i++) {
      ...
      double var_i = pi.phi * (*pi.theta2_sq)[i] * latent_nu[i];
      double weight_i = 1.0 / var_i;
      ...
      sv[ni].sw += weight_i;
      sv[ni].swr += y*weight_i;

      sv[ni].indices.push_back(i);
   }
}
//--------------------------------------------------
//get sufficient stats for children (v,c) of node nx in tree x
void getsuff(tree& x, tree::tree_cp nx, size_t v, size_t c, xinfo& xi, dinfo& di, sinfo& sl, sinfo& sr, pinfo& pi, const std::vector<double>& latent_nu)
{
   ...
   for(size_t i = 0; i < di.n; i++) {
      ...
      if(nx == x.bn(xx,xi)) { // If the point is in the node being split...
         if(xx[v] < xi[v][c]) {
               sl.indices.push_back(i); // ...add its index to the left vector
         } else {
               sr.indices.push_back(i); // ...or the right vector
         }
      }
   }
   ...
   sl.sw=0.0;sl.swr=0.0;
   sr.sw=0.0;sr.swr=0.0;
   ...
   for(size_t i=0;i<di.n;i++) {
      ...
      if(nx==x.bn(xx,xi)) { //does the bottom node = xx's bottom node
         ...
         if(xx[v] < xi[v][c]) {
          ...
          double variance_i = pi.phi * (*pi.theta2_sq)[i] * latent_nu[i];
            double weight_i = 1.0 / variance_i;

            if(xx[v] < xi[v][c]) { //goes to the left child
                sl.sw += weight_i;
                sl.swr += weight_i * y;
...
void getsuff(tree& x, tree::tree_cp nl, tree::tree_cp nr, xinfo& xi, dinfo& di, sinfo& sl, sinfo& sr, pinfo& pi, const std::vector<double>& latent_nu)
{


   // Clear the vectors for the new results
   sl.indices.clear();
   sr.indices.clear();
   ...
   for(size_t i = 0; i < di.n; i++) {
      ...
      if(bn == nl) {
         sl.indices.push_back(i);
      }
      if(bn == nr) {
         sr.indices.push_back(i);
      }
   }
   ...
   for(size_t i=0;i<di.n;i++) {
      ...
      if(bn == nl || bn == nr) {
         ...
         if(bn == nl) { //falls in the left child
               sl.sw += weight_i;
               sl.swr += weight_i * y;
         } else { //falls in the right child
               sr.sw += weight_i;
               sr.swr += weight_i * y;
         }
      }
...
void drmuc(tree& t, xinfo& xi, dinfo& di, pinfo& pi, rn& gen, const std::vector<double>& latent_nu, std::vector<bool>& monotone_flags, bool data_aug)
{
   ...
   allsuff(t,xi,di,bnv,sv, pi, latent_nu); //fill bnv and sv
   ...
   for(tree::npv::size_type i=0;i!=bnv.size();i++) {
      ml=mla;mu=mua;
      conint(ml,mu,bnv[i],t,xi, monotone_flags);
      cnt=0;lMax=-bigval;
      ...
      for(size_t j=0;j!=nm;j++) {
         ...
         if((tmu >= ml) && (tmu<=mu)) {
            ...
            if(data_aug) {
               temp = (sv[i].swr * tmu) - (0.5*sv[i].sw * tmu * tmu);
            }else{
               // Direct ALD Likelihood (using indices)
               double log_rho_sum = 0.0;
               for (size_t data_idx : sv[i].indices) {
...
void conint(double& ml, double& mu, tree::tree_p n, tree& t, xinfo& xi, std::vector<bool>& monotone_flags)
{
   double nmu; //mu for a bottom node
   NeighborRelationship neighbors;
   tree::npv bn;  //bottom nodes
   t.getbots(bn);
   for(size_t i=0;i!=bn.size();i++) {
      nmu = bn[i]->getm();
      if(!(bn[i]==n)) {
         neighbors = n->nhb(bn[i],xi);
         bool below_constrained = std::any_of(neighbors.below_vars.cbegin(), neighbors.below_vars.cend(), [&](size_t index) { return monotone_flags[index]; });
         bool above_constrained = std::any_of(neighbors.above_vars.cbegin(), neighbors.above_vars.cend(), [&](size_t index) { return monotone_flags[index]; });

         if(below_constrained && (nmu>ml)) ml = nmu;
         if(above_constrained && (nmu<mu)) mu = nmu;
      }
   }
}
