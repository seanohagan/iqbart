...
double _rgig_ROU_noshift ( double lambda, double lambda_old, double omega, double alpha, rrn& gen)
/*---------------------------------------------------------------------------*/
/* Tpye 1:                                                                   */
...
/*---------------------------------------------------------------------------*/
{
  ...
  /* -- Generate sample ---------------------------------------------------- */
    ...
    do {
      ++count;
      U = um * runif(0,1, gen);        /* U(0,umax) */
      V = runif(0,1, gen);             /* U(0,vmax) */
      X = U/V;
...
//_rgig_ROU_shift_alt
double _rgig_ROU_shift_alt ( double lambda, double lambda_old, double omega, double alpha, rrn& gen)
/*---------------------------------------------------------------------------*/
/* Type 8:                                                                   */
...
double rv;
  ...
  /* -- Generate sample ---------------------------------------------------- */
    ...
    do {
      ++count;
      U = uminus + runif(0,1, gen)  * (uplus - uminus);    /* U(u-,u+)  */
      V = runif(0,1, gen);                                /* U(0,vmax) */
      X = U/V + xm;
    }                                         /* Acceptance/Rejection */
...
double  _rgig_newapproach1 ( double lambda, double lambda_old, double omega, double alpha, rrn& gen)
/*---------------------------------------------------------------------------*/
/* Type 4:                                                                   */
...
//
// [[Rcpp::export]]
double do_rgig(double lambda, double chi, double psi, rrn& gen){
    double rv ;
  ...
  if ( !(std::isfinite(lambda) && std::isfinite(chi) && std::isfinite(psi)) ||
       ...
       (chi == 0.0 && lambda <= 0.0) ||
       (psi == 0.0 && lambda >= 0.0) ) {
    {throw std::invalid_argument(" invalid GIG arguments\n"); }
    }

  if (chi < ZTOL) {
    /* special cases which are basically Gamma and Inverse Gamma distribution */
    if (lambda > 0.00) {
      rv =  rgamma(lambda, 2.0/psi, gen);
    }
    else {
      rv = 1.0/rgamma(-lambda, 2.0/psi, gen);
    }
  }
    ...
    /* special cases which are basically Gamma and Inverse Gamma distribution */
    if (lambda > 0.0) {
      rv = 1.0/rgamma(lambda, 2.0/chi, gen);
    }
    else {
      rv = rgamma(-lambda, 2.0/chi, gen);
    }
...
//Rcpp::Rcout <<"labmda, omega: "<< lambda << ", " << omega << endl;
    ...
    do {
      if (lambda > 2.0 || omega > 3.0) {
        /* Ratio-of-uniforms with shift by 'mode', alternative implementation */
      rv =   _rgig_ROU_shift_alt(lambda, lambda_old, omega, alpha, gen);
        break;
      }
      ...
      //  Rcpp::Rcout <<  "_rgig_ROU_noshift" << endl;
        rv = _rgig_ROU_noshift(lambda, lambda_old, omega, alpha, gen);
      ...
      if (lambda >= 0.0 && omega > 0.0) {
       ...
       rv =  _rgig_newapproach1(lambda, lambda_old, omega, alpha, gen);
        break;
      }

      /* else */
      throw std::invalid_argument("parameters must satisfy lambda>=0 and omega>0.0 \n") ;
