...
struct NeighborRelationship {
   std::vector<size_t> above_vars;
   std::vector<size_t> below_vars;

   bool empty() const { return above_vars.empty() && below_vars.empty();}
};
...
public:
   ...
   friend bool bdc(tree& x, xinfo& xi, dinfo& di, pinfo& pi, rn& gen, const std::vector<double>& latent_nu, std::vector<bool>& monotone_flags, bool data_aug);
   ...
   NeighborRelationship nhb(tree_p n, xinfo& xi);
