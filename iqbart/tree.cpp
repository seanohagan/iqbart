...
//  each node is a region of the form \cap [Lv,Uv] v=0,1,..,(p-1)
{
   ...
   for(size_t v=0;v<p;v++) {
      ...
      if(oU < mL-2 || oL > mU+2 ) return {};
      // if(oU == mL-2) nind = 1;
      if(oU == mL-2) neighbors.below_vars.push_back(v);
      // if(oL == mU+2) nind = 2;
      if(oL == mU+2) neighbors.above_vars.push_back(v);
   }
   return neighbors;
   // if(nind==1) return 'b';
   // if(nind==2) return 'a';
